...
export type ExtensionMessage =
  | { type: 'render-page'; pageId: string; nodes: SerializedNode[] }
...
export function serializePage(
  graph: SceneGraph,
  pageId: string
): { nodes: SerializedNode[]; truncated: boolean } {
  ...
  if (!truncated) {
    return { nodes: subset.map(serializeNode), truncated: false }
  }
  ...
  return { nodes: serialized, truncated: true }
}
