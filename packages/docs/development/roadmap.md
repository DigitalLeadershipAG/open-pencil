...
- AI chat: OpenRouter direct (no backend), Stronghold key storage, 86 tools defined once in `packages/core/src/tools/`, model selector, ⌘J toggle, streaming markdown, Playwright tests with mock transport
- 60 additional AI/MCP tools ported from figma-use (86 total): granular set tools, node operations, variable CRUD, boolean operations, vector path tools, viewport control
- MCP server (@open-pencil/mcp): stdio + HTTP (Hono + Streamable HTTP with sessions), 86 core tools + 3 file management tools, runs on Bun and Node.js
- Unified tool definitions: define once in `packages/core/src/tools/`, adapt for AI chat (valibot), MCP (zod), CLI (eval)
