...
`@open-pencil/mcp` exposes 86 core tools + 3 file management tools (89 total) for AI coding tools. Two transports: stdio for Claude Code/Cursor/Windsurf, HTTP with Hono + Streamable HTTP for scripts and CI. Tools are defined once and adapted for AI chat (valibot), MCP (zod), and CLI (eval command).
...
The MCP server currently exposes 89 tools. The reference implementation in [figma-use](https://github.com/dannote/figma-use) has 118. The remaining 40 tools cover advanced layout constraints, prototype connections, advanced component property editing, and bulk document operations — all will be ported.
