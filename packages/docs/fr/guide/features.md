...
Assistant IA intégré via <kbd>⌘</kbd><kbd>J</kbd>. **89 outils** dans `packages/core/src/tools/` couvrant lecture, création, modification, manipulation de nœuds, CRUD variables, outils vectoriels, contrôle viewport et `eval`. **Serveur MCP** expose 89 outils au total (86 core + 3 gestion fichiers) pour les outils de codage IA externes.
