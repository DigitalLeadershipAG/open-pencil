...
Assistente IA integrato via <kbd>⌘</kbd><kbd>J</kbd>. **89 strumenti** in `packages/core/src/tools/` che coprono lettura, creazione, modifica, manipolazione nodi, CRUD variabili, strumenti vettoriali, controllo viewport e `eval`. **Server MCP** espone 89 strumenti totali (86 core + 3 gestione file) per strumenti di codifica IA esterni.
